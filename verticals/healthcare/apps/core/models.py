...
class LabTest(models.Model):
    ...
    patient = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="lab_tests"
    )
...
class DoctorVisit(models.Model):
    ...
    patient = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="doctor_visits"
    )
    doctor_name = models.CharField(max_length=100)
    specialty = models.CharField(max_length=100)
    visit_date = models.DateField()
    visit_type = models.CharField(
        max_length=20, choices=VISIT_TYPE_CHOICES, default="checkup"
    )
    ...
    def __str__(self):
        return (
            f"{self.doctor_name} - {self.patient.get_full_name()} ({self.visit_date})"
        )
