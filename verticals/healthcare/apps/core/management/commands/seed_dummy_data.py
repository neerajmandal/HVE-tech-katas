    ...
    def handle(self, *args, **kwargs):
        ...
        for i, (first_name, last_name) in enumerate(patient_names, 1):
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created patient profile for {first_name} {last_name}"
                    )
                )
        ...
        for patient in patients:
            ...
            selected_tests = random.sample(
                lab_tests_data, min(num_tests, len(lab_tests_data))
            )
            ...
            for test_data in selected_tests:
                ...
                if ref_range.startswith("<"):
                    ...
                    result_val = (
                        float(ref_range[1:]) * 1.3 if is_abnormal else normal_val
                    )
                elif ref_range.startswith(">"):
                    normal_val = float(ref_range[1:]) * 1.3
                    result_val = (
                        float(ref_range[1:]) * 0.7 if is_abnormal else normal_val
                    )
                elif "-" in ref_range:
                ...
                LabTest.objects.create(
                    ...
                    notes="Abnormal result - follow up recommended."
                    if is_abnormal and status != "pending"
                    else "",
                )
        ...
        for patient in patients:
            ...
            for _ in range(num_visits):
                ...
                DoctorVisit.objects.create(
                    ...
                    vitals_temperature=Decimal(
                        str(round(random.uniform(97.0, 99.5), 1))
                    ),
        ...
        for i in range(15):
            ...
            status = random.choices(
                ["pending", "overdue", "paid"], weights=[70, 20, 10], k=1
            )[0]
